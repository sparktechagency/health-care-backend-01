...
import { MedicalQuestionQuestion as Question } from './question.model';
import { ImedicalQuestion } from './question.interface';

const createQuestion = async (payload: ImedicalQuestion): Promise<ImedicalQuestion> => {
...
const getQuestionById = async (id: string): Promise<ImedicalQuestion | null> => {
...
const updateQuestion = async (
  id: string,
  payload: ImedicalQuestion
): Promise<ImedicalQuestion | null> => {
...
const deleteQuestion = async (id: string): Promise<ImedicalQuestion | null> => {
  const isExistQuestion = await getQuestionById(id);
  if (!isExistQuestion) {
    throw new ApiError(StatusCodes.BAD_REQUEST, 'Question not found!');
  }
